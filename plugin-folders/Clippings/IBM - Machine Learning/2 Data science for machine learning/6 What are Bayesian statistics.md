...
source: https://www.ibm.com/think/topics/bayesian-statistics
author:
- '[[Joshua Noble]]'
...
- $\theta$ represents a hypothesis

- $\mathbf{D}$ is the observed data

- $p(\theta)$ is the prior probability distribution which expresses beliefs about θ before seeing the data. These can be based upon previous research, or the experimental set up in the case of an experiment. This makes the impact of the rest of the world outside of the data generating process explicit in the calculation.

- $p(\mathbf{D} \mid \theta)$ is the likelihood function and it represents the probability of observing the data given the hypothesis

- $p(\mathbf{D})$ is the evidence (sometimes called ‘marginal likelihood’) that serves as a normalizing constant in the equation. This is how likely the observed data is, independent of any testing.

- $p(\theta \mid \mathbf{D})$ this is the posterior distribution, the updated belief about the hypothesis after incorporating the evidence and the priors.

Bayes theorem enables inference in the form of a posterior distribution that can be used to compute point estimates like the mean, mode, or median of the posterior. The posterior distribution also contains interval estimates as well as predictive distributions for new data.<sup>[1](#f1)</sup>
...
![The observed distribution and prior distribution being combined into a posterior distribution](https://assets.ibm.com/is/image/ibm/prior-posterior-data?ts=1763388322582&dpr=off)   Observed distribution and prior distribution combine to create a posterior distribution
...
In Frequentist statistics a p-value is commonly used to represent how well a model has estimated a parameter value. Technically a p-value is the probability of observing data as extreme as or more extreme than the observed, assuming that the null hypothesis ( $H_0$ ) is true.

p-value = P(Data as extreme as observed | $H_0$ )

The p-value is not the probability that ( $H_0$ ) is true, it measures how surprising the data would be if ( $H_0$ ) were true. Small p-values (e.g., < 0.05) suggest the data are inconsistent with ( $H_0$ ), but they don’t directly compare ( $H_0$ ) to an alternative. This is why Frequentist statistics is often described as Null Hypotheses Testing because it enables comparing any hypotheses to the null hypotheses.
...
$$ y_i = \beta_0 + \beta_1 x_i + \varepsilon_i, \quad \varepsilon_i \sim \mathbf{N}(0, \sigma^2) $$

The following variables are represented in this model:

- $\beta_0$ = intercept
- $\beta_1$ = slope
- $\epsilon$ = noise variance. One of the assumptions of linear regression is that the error is normally distributed with a mean of 0 and a standard deviation of $\sigma^2$
...
A classically frequentist approach would treat $\beta_0$ , $\beta_1$ as fixed but unknown values. An algorithm like least squares would be used to minimize the error in the estimation of $\beta_0$ , $\beta_1$ :

$$ \hat{\beta}_1 = \frac{\sum (x_i - \bar{x})(y_i - \bar{y})}{\sum (x_i - \bar{x})^2} $$

$$ \hat{\beta}_0 = \bar{y} - \hat{\beta}_1 \bar{x} $$

This would produce a single point estimate for each $\beta_0$ and $\beta_1$ . Any uncertainty in those estimates would be described indirectly by confidence intervals and standard errors.

### Bayesian Estimation

When estimating the relationship between study hours and exam scores using Bayesian statistical methods, the data scientist would treat each of $\beta_0$ , $\beta_1$ and $\sigma^2$ as random variables. They would specify priors for each of those values:

$$ \beta_0 \sim \mathbf{N}(0, 10^2) $$

$$ \beta_1 \sim \mathbf{N}(0, 10^2) $$

$$ \sigma^2 \sim Inverse-Gamma(1, 1) $$

This states that the data scientist believes that $\beta_0$ and $\beta_1$ are somewhere within a normal distribution centered around 0 and that the errors are probably small but never negative. The inverse-gamma distribution models this belief because it has 0 as a lower bound and the majority of the probable values below 2.

![The inverse gamma distribution, which is often as a prior for errors.](https://assets.ibm.com/is/image/ibm/inverse-gamma-distribution?ts=1763388324787&dpr=off)   The inverse gamma distribution, often as a prior for errors.

The data would be used to modify those parameters

$$ p(y \mid \beta_0, \beta_1, \sigma^2, x) = \prod_i \mathbf{N}(y_i \mid \beta_0 + \beta_1 x_i, \sigma^2) $$

The Posterior then is derived via Bayes’ theorem:

$$ p(\beta_0, \beta_1, \sigma^2 \mid data) \propto 
p(y \mid \beta_0, \beta_1, \sigma^2, x) \cdot p(\beta_0) \cdot p(\beta_1) \cdot p(\sigma^2) $$
...
$$ $\hat{y} = \hat{\beta}_0 + \hat{\beta}_1 \cdot 10 $$

The Bayesian model would sample from the posterior of $\beta_0$ , $\beta_1$ , $\sigma^2$ and generate a distribution of predicted scores for 10 hours of study. The more informative and aligned that the data and the priors are, the narrower that generated distribution would be.
...
An individual student i in school j would be modeled by a normal distribution shaped by the information we know about all students $\sigma^2$ and about the school that the student attends $\theta_j$ :

$$ (y_{ij} \mid \theta_j, \sigma^2) \sim \mathbf{N}(\theta_j, \sigma^2) $$

Each school would be modeled by a normal distribution shaped by using what we know about all students and what we know about that school $\tau^2$

$$ (\theta_j \mid \mu, \tau^2) \sim \mathbf{N}(\mu, \tau^2) $$

Finally, all students are modeled using a population level information:

$$ \mu \sim \mathbf{N}(0, \sigma_\mu^2) $$

This states that the population mean is near 0, but uncertain with scale $\sigma_\mu^2$
